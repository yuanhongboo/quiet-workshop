import { bakeArtwork } from '../src/bake-art.mjs';
import { bookArtwork } from '../src/book-art.mjs';
...
const presentations={
  'evening-bakery':{theme:'bake',note:'晚风经过，面包正暖。',caption:'THE EVENING BAKERY',description:'擦亮旧烤箱，揉圆一团柔软。让新出炉的面包，温暖街角的窗。'},
  'hillside-library':{theme:'book',note:'风吹书页，故事留下。',caption:'PAGES IN THE BREEZE',description:'补好一本旧书，点亮一束幻灯。让走过的花房、灯塔与小站，留在这间书屋。'},
...
export function seasonCardsMarkup(seasons,releaseUrl='../dist/') {
  return seasons.map(season=>{
    ...
    const art=bakeArtwork(season.openingId)||bookArtwork(season.openingId)||stationArtwork(season.openingId)||postArtwork(season.openingId)||gardenArtwork(season.openingId)||seasonArtwork(season.openingId)||'';
...
export function latestSeasonMarkup(seasons, releaseUrl='../dist/') {
  const season=seasons.at(-1);
  return season ? `<a id="latest-season-link" class="latest-season" href="${escape(`${releaseUrl}?season=${encodeURIComponent(season.id)}`)}"><span>新的一季</span>${escape(seasonLabel(season))} · ${escape(season.title)} ↗</a>` : '';
}
