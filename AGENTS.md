...
Run relevant deterministic tests, then the full existing suite and production build before promoting shared changes. For changed interactions, verify actual browser input and the route to completion; test a narrow phone viewport and report it separately from real hardware testing. Use a separate browser origin/port for fixtures, never overwrite player saves. Visibility tests must use real initial Rapier poses and runtime scene-update ordering; never hide loose items only in a test to bypass an obstruction. Finish cleaning before advancing completion tasks and assert the tested scene is actually completed. Check artwork facing direction as well as model bounds. Check season switching, reload/resume and collection isolation when those boundaries change.
