...
import { ALL_LEVELS, GARDEN_LEVELS, POST_LEVELS, STATION_LEVELS, BOOK_LEVELS, BAKE_LEVELS, LEVELS, getLevel, nextLevel } from '../src/levels.mjs';
...
test('all catalogs keep stable earlier IDs and route 54 levels within their own season',()=>{
 assert.equal(LEVELS.length,9);assert.equal(GARDEN_LEVELS.length,9);assert.equal(POST_LEVELS.length,9);assert.equal(STATION_LEVELS.length,9);assert.equal(BOOK_LEVELS.length,9);assert.equal(BAKE_LEVELS.length,9);assert.equal(new Set(ALL_LEVELS.map(l=>l.id)).size,54);
 ...
 assert.equal(nextLevel('opening').id,'coffee');assert.equal(nextLevel('garden-awakening').id,'garden-pot');assert.equal(nextLevel('post-opening').id,'post-box');assert.equal(nextLevel('station-opening').id,'station-sign');assert.equal(nextLevel('book-opening').id,'book-cover');assert.equal(nextLevel('bake-opening').id,'bake-oven');
});
