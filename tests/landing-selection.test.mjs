...
import { SEASON, SEASON_TWO, SEASONS, seasonLevelIds } from '../src/season.mjs';
...
test('new visitors start each season in its first unlocked level',()=>{for(const season of SEASONS)assert.equal(landingSelection(season,rows(season),'unknown').id,seasonLevelIds(season)[0]);});
